...
public class MongoConnection {
    ...
    public int getRank(String uuid, String key) {
        List<IntegerValueData> values = new ArrayList<>();
        for (Document document : getPlayers().find().sort(Sorts.orderBy(Sorts.descending(key)))) {
            values.add(new IntegerValueData(document.getString("UUID"), document.getInteger(key)));
        }
        for (int i = 0; i < values.size(); i++) {
            if (values.get(i).getName().equals(uuid)) {
                return i + 1;
            }
        }
        return 0;
