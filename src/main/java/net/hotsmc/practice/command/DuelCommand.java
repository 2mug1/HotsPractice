...
import net.hotsmc.practice.gui.ladder.DuelPartyLadderMenu;
import net.hotsmc.practice.gui.ladder.DuelPlayerLadderMenu;
...
public class DuelCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        if(commandSender instanceof Player) {
            ...
            if(practicePlayer.isInMatch() || practicePlayer.isInEvent() || practicePlayer.isEnableSpectate() || practicePlayer.isEnableKitEdit() || practicePlayer.isInQueue()){
            ...
            if(args.length == 0){
                ...
                practicePlayer.sendMessage(ChatColor.YELLOW + "/duel <player/party_leader>");
                practicePlayer.sendMessage(ChatColor.YELLOW + "/duel accept <player/party_leader>");
            }
            ...
            if(args.length == 2){
                if(args[0].equalsIgnoreCase("accept")){
                    ...
                    if(practicePlayer.isInParty()){
                        ...
                        DuelMatchRequest duelMatchRequest = practicePlayer.getDuelGameRequestBySender(targetPracticePlayer);

                        //Duelが送られていたら
                        if(duelMatchRequest != null){
                            //相手がパーティに属していたら
                            if (targetPracticePlayer.isInParty()) {
                            ...
                            duelMatchRequest.accept();
                        }else{
                            practicePlayer.sendMessage(ChatColor.RED + targetName + " hasn't sent duel to you yet / " + targetName + "はあなたにDuelを送っていません");
                        }
                    }
                }
            }
